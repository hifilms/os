...
// Wake Lock to keep connection alive on mobile
let wakeLock = null;
...
async function init() {
    try {
        ...
        if (myProfile) {
            ...
            // Handle background sync and visibility
            document.addEventListener('visibilitychange', () => {
                if (document.visibilityState === 'visible') {
                    console.log('App in foreground, checking connectivity...');
                    checkContactsAvailability();
                    flushAllPendingMessages();

                    // Reconnect peer if needed
                    if (peerManager.peer && (peerManager.peer.disconnected || peerManager.peer.destroyed)) {
                        initNetwork(myProfile.mobile);
                    }
                }
            });

            // Periodically check for pending messages (every 10 seconds)
            setInterval(flushAllPendingMessages, 10000);
        ...
        // Fallback to show login if database fails
...
function initNetwork(id) {
    ...
    peerManager.onMessage(async (data, fromPeerId) => {
        ...
        const msgId = await db.addMessage(msg);
        msg.id = msgId; // Capture the ID for immediate actions like delete
...
async function flushAllPendingMessages() {
    ...
    for (const contact of contacts) {
        const pendingMsgs = await db.getPendingMessages(contact.mobile);
        if (pendingMsgs.length > 0) {
            console.log(`Auto-flushing ${pendingMsgs.length} messages to ${contact.mobile}`);
            // Try to connect if offline
            if (!onlinePeers.has(contact.mobile)) {
                peerManager.connect(contact.mobile);
            }

            for (const m of pendingMsgs) {
                const sent = peerManager.sendMessage(contact.mobile, {
                    type: m.type,
                    timestamp: m.timestamp,
                    content: m.content,
                    file: m.file,
    ...
    // ONLY refresh if something was actually sent to the currently open chat
    if (anySent && currentChatPeerId) {
        updateMessageStatusIcons();
    }
    loadContactsDebounced();
}

// New helper to update icons without full refresh
async function updateMessageStatusIcons() {
    if (!currentChatPeerId) return;
    const messages = await db.getMessages(currentChatPeerId);
    const messageEls = chatMessages.querySelectorAll('.message.sent');

    messages.forEach(msg => {
        if (!msg.isMine) return;
        // Find the message element by timestamp (since we don't have unique IDs in DOM)
        // This is a simple heuristic. Better would be data-id.
        const el = Array.from(messageEls).find(e => e.dataset.timestamp == msg.timestamp);
        if (el) {
            const iconWrap = el.querySelector('.msg-status-icon');
            if (iconWrap) {
                const newIcon = STATUS_ICONS[msg.status] || '';
                if (iconWrap.innerHTML !== newIcon) {
...
// Avatar & Cropping with Compression for 2G/3G
...
const handleAvatarChange = (e, previewId) => {
    ...
    if (file) {
        ...
        reader.onload = async (event) => {
            ...
            // Compress before saving and sending
            const compressed = await compressImage(dataUrl);
            currentAvatarBase64 = compressed;
            forceSetAvatar(previewId, compressed);
        };
...
async function forwardMessage(msg, targetPeerId, targetContact) {
    ...
    const id = await db.addMessage(fMsg);
    fMsg.id = id; // Set ID for immediate deletion support
    loadContacts();
...
msgInput.oninput = () => {
    ...
    // Proactively try to connect while user is typing...
...
document.getElementById('btn-attach-menu').onclick = (e) => {
    e.stopPropagation();
    openSheet('sheet-attachment');
};
...
function updateInputButtons() {
    ...
    // Clear any previous inline styles to avoid !important conflicts
...
async function startRecording() {
    try {
        ...
        mediaRecorder.onstop = async () => {
            ...
            if (audioBlob.size > 1000) { // Only send if more than 1 second approx
...
async function sendMessage(type, file = null, caption = '') {
    if (!currentChatPeerId) {
    ...
    let content = '';
    if (type === 'text') content = msgInput.value.trim();
    else if (type === 'audio') content = 'Audio Message';
    else if (type === 'file') content = caption || file?.name || 'File';
    ...
    try {
        const msgId = await db.addMessage(sMsg);
        sMsg.id = msgId; // CAPTURE THE ID so delete works immediately!
        console.log(`Message saved to DB with ID: ${msgId}`);
    } catch (dbErr) {
...
async function refreshMessages() {
    // Save current scroll position to check if we are at bottom
    const isAtBottom = chatMessages.scrollHeight - chatMessages.scrollTop <= chatMessages.clientHeight + 50;

    const messages = await db.getMessages(currentChatPeerId);
    messages.sort((a, b) => a.timestamp - b.timestamp);

    // If message count is same, maybe only statuses changed?
    const currentMsgCount = chatMessages.querySelectorAll('.message').length;
    if (currentMsgCount === messages.length && currentMsgCount > 0) {
        updateMessageStatusIcons();
        return;
    }
    ...
    else setTimeout(scrollToBottom, 300);
}
...
function appendMessageToUI(msg, shouldScroll = true) {
    ...
    div.dataset.timestamp = msg.timestamp; // Add timestamp to DOM for easier updates

    let content = msg.content;
    if (msg.type === 'file' || msg.type === 'audio') {
        const fileData = msg.file;
        const fileType = msg.fileType || (msg.type === 'audio' ? 'audio/webm' : '');
        const url = URL.createObjectURL(new Blob([fileData], { type: fileType }));

        if (fileType?.startsWith('image/')) {
            content = `
                <div class="img-msg-wrap">
                    <img src="${url}" class="file-preview" onload="scrollToBottom()">
                    ${msg.content && msg.content !== msg.fileName ? `<div class="caption">${linkify(msg.content)}</div>` : ''}
                </div>
            `;
        } else if (msg.type === 'audio' || fileType?.startsWith('audio/')) {
            const audioId = `audio-${msg.timestamp}`;
            content = `
                <div class="audio-player" id="${audioId}-container">
                    <button class="audio-btn" onclick="toggleAudio('${url}', '${audioId}')" id="${audioId}-btn">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
                    </button>
                    <div class="audio-waveform" onclick="seekAudio(event, '${audioId}')">
                        <div class="audio-progress" id="${audioId}-progress"></div>
                    </div>
                    <span class="audio-time" id="${audioId}-time">0:00</span>
                    <audio id="${audioId}" src="${url}" ontimeupdate="updateAudioProgress('${audioId}')" onended="resetAudioUI('${audioId}')"></audio>
                </div>`;
        }
        else content = `<div class="file-card"><div class="file-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline></svg></div><div style="flex:1;overflow:hidden;"><div style="font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${msg.fileName}</div><div style="font-size:10px;color:rgba(255,255,255,0.5);">${(msg.fileName || 'FILE').split('.').pop().toUpperCase()} file</div></div></div>`;
    } else {
...
window.updateAudioProgress = (id) => {
    ...
    time.textContent = `${curMins}:${curSecs.toString().padStart(2, '0')}`;
};
...
window.resetAudioUI = (id) => {
    ...
    document.getElementById(`${id}-time`).textContent = '0:00';
};
